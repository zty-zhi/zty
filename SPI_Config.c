...
void  SPI1_WriteByte(uint8_t TxData)   //SPI  写字节函数 
{		
				 	
	while (SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_TXE) == RESET); //等待发送数据空标志位清空
...
{		
	...
	while (SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_TXE) == RESET); //等待发送数据空标志位清空
